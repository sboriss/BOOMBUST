...
if( do_vint$GDP ){
    ...
    info_incpt         = c( sx = "INCPT"               , sxs = "incpt"       , publag = "1", transf = "m2q_lvl" )
...
if( do_vint$TSX){      
      ...
      lapply( list_info_tsx, function(info_tsx){ #  info_tsx = list_info_tsx[[35]]; info_tsx
        ...
        sapply(list_fcst_orgn, function(j_fcst_orgn){ # 
          
          j_fcst_orgn = list_fcst_orgn[157]; j_fcst_orgn
            ...
            tfplot( mtsx_vntg[,1] ); lines( mtsx_vntg[,2]); lines( mtsx_vntg[,3])

            head( mtsx_vntg )
            
            mtsx_vntg_csv = paste0(path$vntg, "tsx_",info_tsx[ "sxs" ],"_",j_fcst_orgn,".csv")
            tsWrite( mtsx_vntg, mtsx_vntg_csv )
...
if( FALSE ){ 
    ...
    list_model_choice     = c( list_model_choice, "AR2+incpt+vacancy_yoy+omxr_qoq",
                               ...
                               "AR2+incpt+vacancy_yoy+omxr_qoq+survreta_lvl+retail_yoy")
    ...
    list_lmbd_set  = c( "diffuse" ,"as_in_CCM2015") #   
    list_SV_switch = c( "SV-OFF","SV-ON")
    
    fcst_orgn = opt$fcst_orgn; fcst_orgn
          ...
          for( sv in seq_along(list_SV_switch) ){
            ...
            SV_switch = list_SV_switch[ sv ]; SV_switch
              ...
              for(j_mdl in seq_along(list_model_choice) ){ # j_mdl = 1 
                ...
                model_choice = list_model_choice[ j_mdl ]; print( model_choice )
                
                # SV_switch    = "SV-ON"; model_choice = "AR2+incpt+survbuid_diq"
                  ...
                  list_mtsyx_fo = lapply( list_fcst_trgt, function(j_fcst_trgt_date){ # j_fcst_trgt_date = list_fcst_trgt[56]; j_fcst_trgt_date 
                      ...
                      j_fcst_orgn_date = j_fcst_trgt_date + months( gsub( "fo","",opt$fcst_orgn ) %>% as.integer - 3 ); j_fcst_orgn_date 
                      
                      y_vntg = list.files( path$vntg, pattern = paste(j_fcst_orgn_date, opt$fcst_orgn, sep = "_" ) )
                      ...
                      list_dbyx = lapply( c( y_vntg, x_vntg_all[ model_choice_sel ] ), function(x_vntg){ #x_vntg = x_vntg_all[ model_choice_sel ][1]; x_vntg
                      ...
                      tsx_fcst = tsx_fcst_aux[ , tsx_name_incpt_first, drop = F ]; tsx_fcst
                      
                      tsyx_estn = na.omit( mtsyx_vntg )
                      
                      tsy_estn = tsyx_estn[, opt$sy  , drop = F]
                      tsx_estn = tsyx_estn[, tsx_name_incpt_first, drop = F]
                  ...
                  fcst_summ = foreach( i = seq_along( names( list_mtsyx_fo ) ), .combine = "rbind", .inorder = T, 
                        ...
                        if( lmbd_set == "as_in_CCM2015"){ lmbd <- c(     0.2,     0.2, 1 ) }  # shrinkage hyperparameters
                  ...
                  tfplot( ts(0, start = start( tsfcst_summ), end = end(tsfcst_summ), frequency = frequency( tsfcst_summ ) ), 
                          title = fcst_title, ylab = "", ylim = range( tsfcst_summ[ ,c( "otrn", "Q16.5%", "Q83.5%" ) ] ), col = "gray" )
                  lines( tsfcst_summ[ ,"otrn"], lwd = 2, type = "b" )
                  lines( tsfcst_summ[ ,"Q50%"], col = "red", lwd = 2, type = "b" )
                  lines( tsfcst_summ[ ,"Q16.5%"], col = "blue", lwd = 2 )
                  lines( tsfcst_summ[ ,"Q83.5%"], col = "blue", lwd = 2 )
                  dev.off()
                  ...
                  rmsfe = sqrt( mean( fcst_summ[,"erro.Q50%"]^2 ) ); rmsfe
...
choicePR = "_as_in_CCM2015" # "_diffuse" # 
